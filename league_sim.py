...
def get_league_data(league):
    ...
    cols = [
        'Wk','Date','Time','Home','xG_home','Score','xG_away','Away',
...
def weighted_team_results(data, alpha=0.8):
    """
...
def match_probabilities_dict(weighted_df):
    """
...
def expected_league_table(team_df, prob_lookup, remain_df):
    """
...
def simulate_position_probabilities(team_df, remain_df, prob_lookup,
                                    target_pos, higher=True, n_sims=10000):
    """
...
def build_weighted_df_from_subset(played_subset_df, alpha=0.98):
    return weighted_team_results(played_subset_df, alpha=alpha)

def find_best_alpha(data, alphas=np.linspace(0.90, 1.00, 11)):
    """
    ...
    for alpha in alphas:
        wdf = build_weighted_df_from_subset(train, alpha=alpha)
        prob = match_probabilities_dict(wdf)
        actual = test.apply(lambda r: match_result(r.home_score, r.away_score), axis=1).values
        preds = np.array([
...
if __name__ == "__main__":
    ...
    alpha_star = find_best_alpha(data, np.linspace(0.8, 1.0, 10000))

    weighted_df = weighted_team_results(data, alpha=alpha_star)
    prob_lookup = match_probabilities_dict(weighted_df)
    team_df = team_results(data)
    remain_df = remaining_matches(data)

    exp_table = expected_league_table(team_df, prob_lookup, remain_df)
    print("----------------------------------------------------")
